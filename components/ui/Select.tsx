    ...
    if (isOpen) {
      ...
      triggerRef.current?.measure((x, y, width, height, pageX, pageY) => {
        ...
        const dropdownHeight = Math.min(
          items.length * DynamicSize(40),
          DynamicSize(300)
        ); // Limit max height to 300
        const dropdownWidth = width + DynamicSize(dropdownPadding ?? 40);
      ...
      >
        ...
        >
          {dropdownPosition && (
            ...
            >
              <ScrollView style={{ maxHeight: DynamicSize(300) }}>
                {items.map((item) => (
                  <TouchableOpacity
                    key={item.id}
                    style={[
...
const styles = StyleSheet.create({
  ...
  modalOverlay: {
    flex: 1,
    marginTop: Platform.OS === 'ios' ? DynamicSize(10) : -DynamicSize(30),
  },
  dropdownList: {
    position: "absolute",
    backgroundColor: "white",
